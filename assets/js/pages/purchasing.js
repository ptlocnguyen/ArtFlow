(function () {
  function create(runtime) {
    ...
    function openSupplierDetail(supplierId) {
      ...
      drawer.innerHTML = `<header class="drawer-header"><div><p class="section-kicker">${escapeHtml(supplier.code)}</p><h2>${escapeHtml(supplier.name)}</h2><small>${escapeHtml(supplier.phone || "Chưa có điện thoại")}</small></div><button class="icon-button" type="button" data-close-management-drawer aria-label="Đóng" title="Đóng">${icon("close")}</button></header><section class="drawer-metrics"><article><small>Tổng mua</small><strong>${money.format(supplier.totalPurchased)}</strong></article><article class="${supplier.outstanding > 0 ? "has-payable" : ""}"><small>Phải trả</small><strong>${money.format(supplier.outstanding)}</strong></article><article><small>Dư có</small><strong>${money.format(supplier.creditBalance)}</strong></article><article><small>Mua gần nhất</small><strong>${supplier.lastPurchaseAt ? formatDate(supplier.lastPurchaseAt) : "Chưa có"}</strong></article></section><section class="drawer-section supplier-contact-detail"><h3>Thông tin liên hệ</h3><span><small>Điện thoại</small><strong>${escapeHtml(supplier.phone || "Chưa có")}</strong></span><span><small>Email</small><strong>${escapeHtml(supplier.email || "Chưa có")}</strong></span><span><small>Địa chỉ</small><strong>${escapeHtml(supplier.address || "Chưa có")}</strong></span><span><small>Mã số thuế</small><strong>${escapeHtml(supplier.taxCode || "Chưa có")}</strong></span>${supplier.note ? `<span><small>Ghi chú</small><strong>${escapeHtml(supplier.note)}</strong></span>` : ""}</section><section class="drawer-section"><h3>Phiếu mua gần đây</h3><div class="supplier-detail-orders">${orders.length ? orders.map(order => `<a href="./purchasing.html?purchaseOrderId=${encodeURIComponent(order.id)}"><span><strong>${escapeHtml(order.code)}</strong><small>${formatDate(order.createdAt)} · ${statusLabel(order.status)}</small></span><b>${money.format(order.netTotal)}</b></a>`).join("") : `<div class="empty compact">Chưa phát sinh phiếu mua.</div>`}</div></section><div class="supplier-drawer-actions"><a class="button ghost" href="./accounting.html?view=receivables&debt=supplier"><span data-icon="calculator"></span> Xem công nợ</a>${canManagePurchasing() ? `<button class="button primary" type="button" data-edit-supplier="${supplier.id}">${icon("edit")} Sửa hồ sơ</button>` : ""}</div>`;
      drawer.hidden = false;
      hydrateIcons(drawer);
    }
